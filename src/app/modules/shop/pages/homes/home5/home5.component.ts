...
export class Home5Component implements AfterViewInit {
  ...
  animationDiv(){
    ...
    if (isVisible) {
      this.renderer.setStyle(this.video, 'transform', `translate3d(0px, 0rem, 0px) scale3d(1, 1, 1) rotateX(${targetDivRect.top*speed}deg) rotateY(0deg) rotateZ(0deg) skew(0deg)`);
    } else {
 ...
 */      // Mover el círculo fuera del viewport si no es visible
  ...
  animationDiv4() {
    ...
    if (isVisible) {
      ...
      const scaleValue = Math.max(1, 1.5 - proportionY * 0.5); // Escala de 2 a 0
/*       const scaleValue = Math.max(0, 2 - proportionY * 2); // Escala de 2 a 0
 */      // Aplicar el movimiento y la escala
      this.renderer.setStyle(this.section4_div1, 'transform', `translateY(${positionY*0.8}px) scale(${scaleValue})`);
    } else {
